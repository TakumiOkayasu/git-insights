...
追跡済みファイルでカーソルを置くと、その行末にBlame（著者・日付・コミット概要）を表示します。行末の注釈にマウスを重ねると、著者・メールアドレス・日時・コミットメッセージ全文・SHAを確認できます。未保存／保存済みの変更行は「未コミットの変更」と表示し、変更のない行は元のコミットを表示します。未追跡ファイルやコミットのないリポジトリでは表示しません。コード上のホバーにはBlame詳細を追加せず、言語機能や他拡張のホバーと共存します。

ホバーはアバター・著者名・相対日時と実日時、コミットメッセージ全文、変更ファイル数と追加／削除行数、短縮SHAと操作リンクを表示します。追加／削除の色はテーマに従います。バイナリは別件数で表示し、行数には加えません。マージの統計は履歴画面と同じく第1親との差分です。SHAのコピーは省略していない値を使います。リモートリンクは既存の対応先 (GitHub / GitLab / Bitbucket) の `origin` がある場合だけ表示します。設定リンクでGit Insightsの設定を開けます。

アバターは履歴画面と同じ `gitInsights.gravatar.enabled` に従い、初期値は無効です。有効時のみ既存の取得処理とキャッシュを共有します。著者・メッセージなどのリポジトリ由来の文字列はエスケープし、ホバーから実行可能なコマンドは専用の3操作だけに限定しています。統計の取得に失敗した場合はその旨を表示し、Blame自体は残します。
