...
VS Code 1.96以降とGitが必要です。VS Codeの拡張機能画面の「…」→「VSIXからのインストール」で `git-insights-0.2.0.vsix` を選んでください。
...
正式公開は `package.json` のバージョンと一致するタグをpushします。初回は現在の `0.2.0` を使用できます。
...
手動公開は **Run workflow** の対象に既存の `v0.2.0` タグを選び、`publish` をONにしてください。ブランチからの公開やバージョン不一致は拒否します。現在は `v数字.数字.数字` の正式版のみ対応しています。
